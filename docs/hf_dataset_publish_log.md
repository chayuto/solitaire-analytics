...
|---|---|---|---|
| 2026-06-27 | [`a03df5cf`](https://huggingface.co/datasets/chayuto/klondike-llm-decisions/commit/a03df5cf3be31aec763b485103d1daa2c2952d2f) | full 28904, clean-raw 12021, clean-lean 12021, 26b-raw 1949, 26b-lean 1949 | +5377 full / +2084 each clean tier / +106 each 26b tier vs `57b3291a`. First push since 2026-06-19; folds in the 2026-06-19 to 06-27 harvest (four ingest batches), all build `0d47c1c` / `hybrid-v1.6`. Ten new 31B wins across the clean tiers: the 06-22 batch (`#0797a2` the cleanest at 42 `tableau_to_tableau`, `#8036eb`, `#21ba6b`), the 06-24 batch (`#a4fba0`/`#24214f`/`#a7373e`), and the 06-27 batch (`#04c63c`, `#2e39b4` a 0-reversal clean line, `#fd9f67`, `#2b238b` a 6-recycle grind); plus two gemini-3.1-flash-lite wins in `full` only (`#f6180f` 331 moves, `#97ce27` 989 moves, the cohort's third and fourth wins). Loss set, all 0 resigns: the 2026-06-21 kill batch (seven exact-proven dead, four 31B `#e4951a`/`#05eb08`/`#a1bc65`/`#a4dca6`, two flash-lite `#1aabb1`/`#0a8c5c`, one MiniMax-M3 `#16291c`), the 2026-06-22 kill batch (`#8db170` 20,920-state search and `#341a16`, both 31B dead), the 2026-06-24 kill batch (`#9805fc` 31B dead, `#17b775` MiniMax killed-while-winnable, `#fc20cc` 26B winnability-indeterminate, the +106 26b bump), and the 2026-06-27 self-terminated trio (`#52c9aa`/`#8a7bcb`/`#a6dbf8`, all 31B exact-proven dead; `#a6dbf8` at 33 fc / 63% is the corpus's deepest proven-dead board). Resign lever now 1 fold vs 28+ proven-dead no-folds. Append-only; integrity gate `--ref local --src hub` OK on all five configs by id and field. |
| 2026-06-19 | [`57b3291a`](https://huggingface.co/datasets/chayuto/klondike-llm-decisions/commit/57b3291a003c04a8c78d3341c43674b3ddf5e45a) | full 23527, clean-raw 9937, clean-lean 9937, 26b-raw 1843, 26b-lean 1843 | +2720 full / +803 each clean tier / +0 26b (flat: no new 26B sessions) vs `a83873ca`. Folds in the 2026-06-15..19 harvest. Five wins, all build `0d47c1c` / `hybrid-v1.6`: four 31B (`#adc679` 132-`tableau_to_tableau` churn record, `#fbfdf8` 6-recycle, `#211fcc`, `#bd2080` the messiest at 5 immediate reversals) plus the SECOND-ever gemini-3.1-flash-lite win `#789f82` (1446 moves, the corpus's longest win; `full` only). Loss set: the 2026-06-15 kill batch (`#8f922e`/`#4062a9`, both 31B exact-proven dead no-folds) and the 2026-06-17 six-session operator kill batch (four structurally dead incl. `#add77c`/`#1514b9` 31B, two killed-while-winnable; zero resigns), which is also the **MiniMax-M3 cohort debut** (`#594680`/`#268ef3`/`#ce2f13`, provider `tokenrouter`, `full` only, kept out of the clean tiers by the `TEACHER_MODEL` filter, no dedicated config yet). Append-only; 24.5MB of new tail shards uploaded. Integrity gate `--ref local --src hub` OK on all five configs by id and field. |
