...
|---|---:|---|---|
| `client_v1_full_corpus_raw` (default) | **23527** | full interaction (prompt + rawResponse + decision blob + call metadata) | failure-mode research, replay, end-to-end audit |
| `client_v1_teacher_clean_raw` | **9937** | full interaction | fine-tuning, honest training-quality subset (single teacher model, current schema, non-stalled) |
| `client_v1_teacher_clean_lean` | **9937** | derived per-decision (flat schema; see *Fields*) | quick analytics, lightweight loading, headline-statistics work |
...
full = load_dataset("chayuto/klondike-llm-decisions")  # 23527 rows

# The training-friendly subset (filtered, single teacher)
clean_raw  = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_raw")   # 9937 rows
clean_lean = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_lean")  # 9937 rows, flat schema
...
- **Collection window**: 2026-05-17 to 2026-06-19
- **Sessions**: 134 distinct game sessions
- **Models**: `gemma-4-31b-it` (19331), `gemma-4-26b-a4b-it` (1843), `gemini-3.1-flash-lite` (1661), `MiniMax-M3` (692)
- **Schema tiers**: current (23156), legacy (371)
...
|---|---:|---:|
| `draw_card` | 11673 | 50% |
| `tableau_to_tableau` | 6685 | 28% |
| `tableau_to_foundation` | 2060 | 9% |
| `discard_to_tableau` | 1225 | 5% |
| `discard_to_foundation` | 949 | 4% |
| `recycle_stock` | 935 | 4% |
