...
|---|---:|---|---|
| `client_v1_full_corpus_raw` (default) | **25341** | full interaction (prompt + rawResponse + decision blob + call metadata) | failure-mode research, replay, end-to-end audit |
| `client_v1_teacher_clean_raw` | **10227** | full interaction | fine-tuning, honest training-quality subset (single teacher model, current schema, non-stalled) |
| `client_v1_teacher_clean_lean` | **10227** | derived per-decision (flat schema; see *Fields*) | quick analytics, lightweight loading, headline-statistics work |
...
full = load_dataset("chayuto/klondike-llm-decisions")  # 25341 rows

# The training-friendly subset (filtered, single teacher)
clean_raw  = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_raw")   # 10227 rows
clean_lean = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_lean")  # 10227 rows, flat schema
...
- **Collection window**: 2026-05-17 to 2026-06-21
- **Sessions**: 141 distinct game sessions
- **Models**: `gemma-4-31b-it` (20467), `gemini-3.1-flash-lite` (2220), `gemma-4-26b-a4b-it` (1843), `MiniMax-M3` (811)
- **Schema tiers**: current (24970), legacy (371)
...
|---|---:|---:|
| `draw_card` | 12422 | 49% |
| `tableau_to_tableau` | 7484 | 30% |
| `tableau_to_foundation` | 2094 | 8% |
| `discard_to_tableau` | 1320 | 5% |
| `recycle_stock` | 1037 | 4% |
| `discard_to_foundation` | 984 | 4% |
