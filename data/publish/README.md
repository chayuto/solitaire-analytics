...
|---|---:|---|---|
| `client_v1_full_corpus_raw` (default) | **27393** | full interaction (prompt + rawResponse + decision blob + call metadata) | failure-mode research, replay, end-to-end audit |
| `client_v1_teacher_clean_raw` | **11270** | full interaction | fine-tuning, honest training-quality subset (single teacher model, current schema, non-stalled) |
| `client_v1_teacher_clean_lean` | **11270** | derived per-decision (flat schema; see *Fields*) | quick analytics, lightweight loading, headline-statistics work |
...
full = load_dataset("chayuto/klondike-llm-decisions")  # 27393 rows

# The training-friendly subset (filtered, single teacher)
clean_raw  = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_raw")   # 11270 rows
clean_lean = load_dataset("chayuto/klondike-llm-decisions", "client_v1_teacher_clean_lean")  # 11270 rows, flat schema
...
- **Collection window**: 2026-05-17 to 2026-06-24
- **Sessions**: 153 distinct game sessions
- **Models**: `gemma-4-31b-it` (22056), `gemini-3.1-flash-lite` (2360), `gemma-4-26b-a4b-it` (1949), `MiniMax-M3` (1028)
- **Schema tiers**: current (27022), legacy (371)
...
|---|---:|---:|
| `draw_card` | 13083 | 48% |
| `tableau_to_tableau` | 8278 | 30% |
| `tableau_to_foundation` | 2402 | 9% |
| `discard_to_tableau` | 1449 | 5% |
| `recycle_stock` | 1096 | 4% |
| `discard_to_foundation` | 1085 | 4% |
