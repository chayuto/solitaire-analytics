...
posttune_v5b_at*.json the v5 sweep writes into baseline_n20_gemma4_text/.
...
def main() -> None:
    ...
    for ckpt in (250, 500, 750, 1000):
        rows[f"v5b iter {ckpt}"] = score_file(RUN_DIR / f"posttune_v5b_at{ckpt}.json", teacher)
    ...
    (RUN_DIR / "learning_curve_v5b.json").write_text(json.dumps(rows, indent=2))
    print(f"\n-> {RUN_DIR / 'learning_curve_v5b.json'}")

    v2u = rows.get("v2 untuned", {})
    v2u_tier = v2u.get("mean_tier", 0)
    v2u_osc = v2u.get("osc_correct", 0)
    best_v5b, best_tier = None, -1
    for name, r in rows.items():
        if name.startswith("v5b iter") and not r.get("missing") and r["mean_tier"] > best_tier:
            best_tier = r["mean_tier"]
            best_v5b = name
    if not best_v5b:
        print("\nNo v5b checkpoints scored yet. Run sweep_v5b_checkpoints.sh first.")
        return

    r = rows[best_v5b]
    print(f"\nReferences: v2-untuned tier {v2u_tier:.2f}, oscillation {v2u_osc}/7")
    print(f"v5 best: {best_v5b} tier {r['mean_tier']:.2f}, oscillation {r['osc_correct']}/{r['osc_n']}, "
    ...
    print()
    if bp1 and bp2:
        print("PROMOTE: won-games filter preserved oscillation AND held tier. Ship v5b trained.")
    elif bp1:
