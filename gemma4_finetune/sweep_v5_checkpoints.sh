...
# adapter source (adapters_v5) and output names (posttune_v5_*) differ.
#
# Run AFTER training produces adapters_v5/000{0250,0500,0750,1000}_adapters.safetensors
# (train_v2.py --config lora_config_v5.yaml). Pre-registration:
...
ADAPTERS=adapters_v5

echo "=== v5 checkpoint sweep ==="
if [ ! -d "$ADAPTERS" ]; then
  echo "[error] $ADAPTERS/ does not exist. Train first:"
  echo "        $VENV_PY train_v2.py --config lora_config_v5.yaml"
  exit 1
fi
ls "$ADAPTERS/"
echo
...
declare -a CHECKPOINTS=(
  "0000250_adapters.safetensors:posttune_v5_at250.json:posttune_v5_at250_responses"
  "0000500_adapters.safetensors:posttune_v5_at500.json:posttune_v5_at500_responses"
  "0000750_adapters.safetensors:posttune_v5_at750.json:posttune_v5_at750_responses"
  "0001000_adapters.safetensors:posttune_v5_at1000.json:posttune_v5_at1000_responses"
)
...
$PROJECT_VENV score_v5_learning_curve.py 2>&1
